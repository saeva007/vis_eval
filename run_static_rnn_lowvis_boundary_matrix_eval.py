#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Lightweight test-set metrics for the Static-RNN boundary-learning matrix.

This is intentionally smaller than ``run_static_rnn_lowvis_eval_journal.py``:
it reuses the same checkpoint loading, preprocessing, threshold, and metric
helpers, but only writes CSV/JSON tables for quick comparison.

Typical remote usage:
  python paper_eval/run_static_rnn_lowvis_boundary_matrix_eval.py \
    --matrix_run_prefix exp_123456_static_rnn_boundary_matrix \
    --device cpu
"""
...
BOUNDARY_EXPERIMENTS: Dict[int, str] = {
    0: "focal_baseline",
    1: "boundary_weight_focal",
    2: "physical_hard_focal",
    3: "boundary_physical_focal",
    4: "ordinal_focal",
    5: "simple_full_boundary_focal",
}
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Evaluate boundary-learning Static-RNN matrix checkpoints with per-class test metrics."
    )
    p.add_argument(
        "--config_json",
    ...
    p.add_argument("--out_dir", default="static_rnn_boundary_matrix_eval_results")
    ...
    p.add_argument("--matrix_run_prefix", default=os.environ.get("LOWVIS_RNN_RUN_PREFIX", ""))
    p.add_argument(
        "--experiments",
        "--matrix_experiments",
        dest="experiments",
        default=os.environ.get("LOWVIS_RNN_EXPERIMENTS", "0:1:2:3:4:5"),
        help="Experiment ids to evaluate, separated by colon/comma/space.",
    )
    p.add_argument("--allow_missing", action="store_true")
    p.add_argument(
        "--no_auto_latest",
        action="store_true",
        help="Require --matrix_run_prefix instead of auto-detecting the newest boundary-matrix prefix.",
    )
...
def parse_id_list(value: str) -> List[int]:
    ...
    for token in (value or "").replace(",", ":").replace(" ", ":").split(":"):
        ...
        if token:
            exp_id = int(token)
            if exp_id not in BOUNDARY_EXPERIMENTS:
                raise ValueError(f"Unknown boundary experiment id={exp_id}; valid ids={sorted(BOUNDARY_EXPERIMENTS)}")
            out.append(exp_id)
    return out or sorted(BOUNDARY_EXPERIMENTS)
...
def discover_latest_prefix(
    ...
    for exp_id in exp_ids:
        name = BOUNDARY_EXPERIMENTS[exp_id]
...
def build_targets(
    ...
    for exp_id in exp_ids:
        name = BOUNDARY_EXPERIMENTS[exp_id]
        run_id = f"{matrix_run_prefix}_{exp_id}_{name}"
        checkpoint = ckpt_dir / f"{run_id}_{stage_tag}_best_score.pt"
        if not checkpoint.exists():
            if allow_missing:
                print(f"[skip] missing checkpoint: {checkpoint}", flush=True)
                continue
            raise FileNotFoundError(f"Missing checkpoint for experiment {exp_id} {name}: {checkpoint}")
...
def evaluate_target(
    ...
    print(f"\n=== Evaluating {experiment_id}: {target.label} ===", flush=True)
    ...
    return overall, class_metric_rows(target.label, target.run_id, experiment_id, cm, decision_meta), confusion_rows(target.label, target.run_id, experiment_id, cm)


def write_markdown_summary(out_dir: Path, overall: pd.DataFrame, per_class: pd.DataFrame, rank_metric: str) -> None:
    lines = ["# Static-RNN Boundary Matrix Test Metrics", ""]
    if rank_metric in overall.columns:
        ranked = overall.sort_values(rank_metric, ascending=False)
        best = ranked.iloc[0]
        lines.append(f"- Best by `{rank_metric}`: `{best['label']}` ({float(best[rank_metric]):.6f})")
        lines.append("")
    show_cols = [
        "experiment_id",
        "label",
    ...
    lines.append("")
    cls_cols = ["experiment_id", "label", "class_name", "support", "predicted", "precision", "recall", "csi", "far"]
    lines.append(simple_markdown_table(per_class, cls_cols))
    lines.append("")
    (out_dir / "boundary_matrix_metrics_summary.md").write_text("\n".join(lines), encoding="utf-8")


def main() -> None:
    args = parse_args()
    args = journal.apply_paper_eval_config(args, "static_rnn_boundary_eval", default_dir=journal.VIS_EVAL_DIR)
    ...
    if not args.matrix_run_prefix and not args.no_auto_latest:
        args.matrix_run_prefix = discover_latest_prefix(ckpt_dir, args.stage_tag, exp_ids) or ""
        if args.matrix_run_prefix:
            print(f"[auto] matrix_run_prefix={args.matrix_run_prefix}", flush=True)
    if not args.matrix_run_prefix:
        raise ValueError("Pass --matrix_run_prefix, or leave auto-detect enabled with matching checkpoints in --ckpt_dir.")
    ...
    targets = build_targets(args.matrix_run_prefix, ckpt_dir, args.stage_tag, exp_ids, args.allow_missing)
    if not targets:
        raise FileNotFoundError("No boundary-matrix checkpoints were selected for evaluation.")

    print("Static-RNN boundary matrix lightweight evaluation", flush=True)
    ...
    id_by_label = {name: exp_id for exp_id, name in BOUNDARY_EXPERIMENTS.items()}
    ...
    overall_path = out_dir / "boundary_matrix_overall_metrics.csv"
    per_class_path = out_dir / "boundary_matrix_per_class_metrics.csv"
    cm_path = out_dir / "boundary_matrix_confusion_counts.csv"
    ...
    run_config = {
        "matrix_run_prefix": args.matrix_run_prefix,
        "experiments": {str(k): BOUNDARY_EXPERIMENTS[k] for k in exp_ids},
        "base": str(base),
        ...
        "outputs": {
            ...
            "summary_md": str(out_dir / "boundary_matrix_metrics_summary.md"),
        },
    }
