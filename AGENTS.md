...
- For `fig9_events_ultralow_lowvis_counts_1x3`, do not trust the hourly filename rank as the display order. Sort panels by the timestamp where `hour_offset == 0`, and when the figure is meant to show the overall event footprint, plot the `*_low_vis_count` series only instead of mixing in Ultra-low/Fog curves.
