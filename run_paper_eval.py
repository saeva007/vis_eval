...
def export_per_sample_table(meta, y_true, y_raw, pred, probs, output_path):
    ...
    Optional: init_time, init_hour, lead_hours when present in meta (S2 per-init datasets).
    """
...
def main():
    ...
    scenario_threshold_rule = "default"
    if season_thresholds is not None or global_thresholds is not None:
        if best_postprocess_mode and "joint" in str(best_postprocess_mode):
            scenario_threshold_rule = "joint"
    scenario_results = plot_scenario_bars(
        meta_full, os.path.join(output_dir, "fig7_scenario_robustness.png"),
        fog_th=args.fog_th, mist_th=args.mist_th,
        threshold_rule=scenario_threshold_rule,
    )
    save_forecast_init_metrics_table(
